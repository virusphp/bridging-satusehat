...
class BridgeKfa extends SimpleCurlFactory
{
	protected $auth;
	...
	{
		$result = $this->request($this->config->setUrlKfa() . $endpoint, "GET", "",  $this->access_token);
		return $result;
	}

	public function postRequest($endpoint, $data)
	{
		$result = $this->request($this->config->setUrlKfa() . $endpoint, "POST", $data,  $this->access_token);
		return $result;
	}

	public function puttRequest($endpoint, $data)
	{
		$result = $this->request($this->config->setUrlKfa() . $endpoint, "PUT", $data,  $this->access_token);
		return $result;
	}
}
