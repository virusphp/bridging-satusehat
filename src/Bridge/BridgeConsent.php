...
class BridgeConsent extends SimpleCurlFactory
{
	protected $auth;
	...
	{
		$result = $this->request($this->config->setUrlConsent() . $endpoint, "GET", "",  $this->access_token);
		return $result;
	}

	public function postRequest($endpoint, $data)
	{
		$result = $this->request($this->config->setUrlConsent() . $endpoint, "POST", $data,  $this->access_token);
		return $result;
	}

	public function puttRequest($endpoint, $data)
	{
		$result = $this->request($this->config->setUrlConsent() . $endpoint, "PUT", $data,  $this->access_token);
		return $result;
	}
}
